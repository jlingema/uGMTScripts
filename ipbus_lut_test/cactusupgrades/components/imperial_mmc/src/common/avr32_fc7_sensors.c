 ...
 * present on the FC7 board.
 *
 */

#if defined(FC7_REV)
#include "imperial_mmc/avr32_fc7_sensors.h"
...
// I2C Bus set-up
static twi_options_t FC7_I2C_OPTS =
{
  .pba_hz = FOSCM,
...
static struct FC7_LTC2990 gMp7Sensors[] =
{
...
#define FC7_NUM_SENSORS (sizeof(gMp7Sensors)/sizeof(struct FC7_LTC2990))
...
void fc7_sensor_init()
{
  dprintf ( "Initialising FC7 Sensors...\n" );
  gCurBus = -1;

  if ( twi_master_init ( &FC7_SENS_TWIM_MODULE, &FC7_I2C_OPTS, FALSE ) )
  {
    dprintf ( "[FC7_SENS] WARNING: Failed to init i2c!\n" );
  }
...
void fc7_sensor_bussel ( U8 pBus )
{
  ...
  if ( twim_write ( &FC7_SENS_TWIM_MODULE, &xSel, 1, 0x70, FALSE ) )
  {
    dprintf ( "Failed to write FC7 sensor bus!\n" );
  }

  //xSel = 0;
  //twim_read(&FC7_SENS_TWIM_MODULE, &xSel, 1, 0x70, FALSE);
  //dprintf("Multiplexer set-up: 0x%02X\n", xSel);
  gCurBus = pBus;
}

U8 fc7_sensor_config ( Bool pAltConfig )
{
  int i;
  U8 xConfig[2];

  // We need to apply the new config to each sensor
  for ( i = 0; i < FC7_NUM_SENSORS; i++ )
  {
    fc7_sensor_bussel ( gMp7Sensors[i].bus );
    xConfig[0] = 0x01; // Write control register on LTC2990
    xConfig[1] = ( pAltConfig ? gMp7Sensors[i].config_b :
                   gMp7Sensors[i].config_a );

    if ( twim_write ( &FC7_SENS_TWIM_MODULE,
                      xConfig, 2,
                      gMp7Sensors[i].addr, FALSE ) )
    {
      dprintf ( "Failed to apply config to FC7 sensor %d!\n", i );
    }
...
U8 fc7_sensor_trigger ( void )
{
  ...
  for ( i = 0; i < FC7_NUM_SENSORS; i++ )
  {
    ...
    if ( twim_write ( &FC7_SENS_TWIM_MODULE,
                      ...
                      gMp7Sensors[i].addr, FALSE ) )
    {
      dprintf ( "Failed to trigger FC7 sensor %d!\n", i );
    }
  }
...
inline float fc7_ltc2990_scale ( float pRaw, float pExp )
{
  return pRaw / 0.1;
}

U8 fc7_ltc2990_read ( U8 pSensor, S8* pTemp,
                      S8* pRes1, S8* pRes2 )
{
  //dprintf("Call to: fc7_ltc2990_read() \n");
  ...
  float xReading1;
  float xReading2;
  fc7_sensor_bussel ( gMp7Sensors[pSensor].bus );

  if ( twim_write ( &FC7_SENS_TWIM_MODULE, &xAddr, 1, gMp7Sensors[pSensor].addr, FALSE ) )
  {
    dprintf ( "Sensor write error (%d)\n", pSensor );
  }

  if ( twim_read ( &FC7_SENS_TWIM_MODULE, xRegs, 12, gMp7Sensors[pSensor].addr, FALSE ) )
  {
                   ...
                   gMp7Sensors[pSensor].config_a ) )
  {
    case 0: // V1, T2
      ...
      xReading1 = fc7_ltc2990_scale ( xReading1, xV1 );
      break;
    case 1: // I1, T2
    ...
    case 2: // I1, V2
      ...
      xReading2 = fc7_ltc2990_scale ( xReading2, xV2 );
      break;
    case 3: // T1, V2
...
U8 fc7_sensor_sht21_run ( void )
{
  // Ask the sensor to read the humidity
  U8 xCmd = 0xF5; // Read Humidity with no hold

  if ( twim_write ( &FC7_SENS_TWIM_MODULE, &xCmd, 1, 0x40, FALSE ) )
  {
...
U8 fc7_sensor_sht21_get ( S8* pHumid )
{
  U8 xRes[3];

  if ( twim_read ( &FC7_SENS_TWIM_MODULE, xRes, 3, 0x40, FALSE ) )
  {
