...
def parse_options():
    ...
    parser.add_argument('-d', '--calo_delay', type=int, dest='calo_delay', default='2', help='Delay of calorimeter sums with respect to muons')
    ...
    for i in range(len(file_list_rx)):
        ...
        if opts.testpath == '':
            ...
            if opts.p5:
                ...
                butlerError += subprocess.call(['mp7butler.py', '-c', opts.connections_file, 'buffers', opts.boardname, 'algoPlay', '--inject', 'file://{fname}'.format(fname=rx_tmp_name), '-e', '0-35', '--play', str(opts.calo_delay)])
                if butlerError != 0:
                ...
                try:
                    ...
                    subprocess.check_output(['mp7butler.py', '-c', opts.connections_file, 'buffers', opts.boardname, 'algoPlay', '--inject', 'file://{fname}'.format(fname=rx_tmp_name), '-e', '0-35', '--play', str(opts.calo_delay)])
